...
void lerEEPROM(void)
{
	...
	while(HAL_I2C_Mem_Read(&hi2c2,  (uint16_t)0xA0, (uint16_t)0x00, sizeof(cmd), eeprom, 256, 100) != HAL_OK);
	//status = HAL_I2C_Mem_Read_DMA(&hi2c2, (uint16_t)0xA1, (uint16_t)0x00, 1, bigbuff, 1);
	UART_print("EEprom\n");
	for(k = 0; k < 256; k++)
    {
		//eepromData[k] = (uint8_t)bigbuff[k];
		UART_print("(#%d)-> %d ", k,  eeprom[k]);
...
//	memset(IRmedia,0,16);
	...
	i = 0;
	UART_print("RAM ANTIGO\n");
	for(int k = 0; k < 64; k++)
    {
		irData[k] = (int16_t) ((buffer[i+1] << 8) | buffer[i]);
		i = i + 2;
		UART_print("%d ", irData[k]);
...
MLX * MLX_Init(I2C_HandleTypeDef * hi2c)
{
	...
	// Config Value + POR Flag
	/*pacote[0] = WRITE_CONFIG_REG;
...
HAL_StatusTypeDef MLX_Read_IT(MLX * mlx) {
	...
	for(int k = 0; k < 64; k++)
	    {
		//UART_print("%d ", (int16_t)mlx->rawIR[k]);
	    // if((k+1)%16 == 0) UART_print("\n");
		}
	//UART_print("\n");
